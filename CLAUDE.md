...
Docker Desktop must be running before starting the AppHost — it launches PostgreSQL (pgvector), Garnet, and RabbitMQ containers and injects their connection strings into the app as `ConnectionStrings__brewhousedb` / `__cache` / `__messaging`. Integration tests reuse the exact same dependency graph via `Aspire.Hosting.Testing`.
...
1. **Synchronize tests (`tests/BrewHouse.Tests/`):**
...
### Proxmox cluster (pre-existing hardware, existing `10.10.10.0/24` LAN)
| Host | IP | Role |
|------|----|------|
| `pve1` | `10.10.10.101` | Cluster master. Hosts the Kemp VM **and** the manually-provisioned `devops` LXC (Terraform + Ansible + the GitHub Actions self-hosted runner, ADR 22). Never a `proxmox_node_1`/`proxmox_node_2` target — no Terraform-managed LXC is ever scheduled here, deliberately, so the box driving `terraform apply` can never be destroyed by it. `proxmox_api_url` points here since the Proxmox API is cluster-aware. |
| `pve2` | `10.10.10.102` | Cluster member, unused by this template — never a `proxmox_node_1`/`proxmox_node_2` target and hosts nothing this repo manages. |
| `pve4` (Node 1 - Primary) | `10.10.10.104` | Hosts every LXC assigned to `proxmox_node_1` below. |
| `pve3` (Node 2 - Secondary) | `10.10.10.103` | Hosts every LXC assigned to `proxmox_node_2` below. |

All four are cluster members, so Proxmox's shared NFS storage (`synology-backups`) mounts on
all four regardless of which two actually host LXCs — see the Synology NAS NFS Permission note
in `LAB-RUNBOOK.md`.

All three are pre-existing physical hosts on the existing LAN, not part of any Terraform-managed
VLAN — same pattern as the Kemp VIP and Synology NAS below.

### VLAN 110 — Web / Ingress (`10.10.110.x`)
...
`10.10.10.0/24` LAN — only the Cloudflared/Web LXCs actually live on VLAN 110 (`10.10.110.x`).

### VLAN 120 — Backend / Data (`10.10.120.x`)
| Host | IP | Node Assignment |
|------|----|-----------------|
| PostgreSQL (pgvector) | `10.10.120.110` | `pve4` (Node 1 - Primary) |
| Microsoft Garnet Cache | `10.10.120.111` | `pve4` (Node 1 - Primary) |
| RabbitMQ | `10.10.120.112` | `pve4` (Node 1 - Primary) |

### VLAN 130 — Management / Infrastructure (`10.10.130.x`)
| Host | IP | Node Assignment |
|------|----|-----------------|
| Infisical (Admin Portal) | `10.10.130.116` | `pve4` (Node 1 - Primary) |
| Uptime Kuma | `10.10.130.117` | `pve3` (Node 2 - Secondary) |
...
| Host | IP | Node Assignment |
|------|----|-----------------|
| PR Preview host (Single Docker Host + Caddy + Ops Stack) | `10.10.140.120` | `pve4` (Node 1 - Primary) |
...
- **PR previews:** `pr-preview.yml` (on PR open/sync, self-hosted runner, `preview` environment) deploys an isolated compose stack per PR to the preview host (`10.10.140.120`); `pr-preview-cleanup.yml` (on PR close) tears it down (`docker compose down -v` + Caddy site removal). Details in `docs/11`.
- **Migrations:** GitHub Actions MUST generate and execute EF Core Migration Bundles BEFORE deploying web apps to prevent race conditions. The same pre-deploy stage runs `dotnet BrewHouse.dll db-apply` to apply Wolverine's `wolverine.*` envelope schema (idempotent; the app itself only self-provisions it in Development).
- **Versioning:** Semantic versioning (MAJOR.MINOR.PATCH) with `version.txt` as the source of truth for MAJOR.MINOR. On every PR opening/reopening against `main`, `bump-minor.yml` auto-bumps the MINOR by 1 (relative to `main`'s current version) and pushes a commit. On deploy (`deploy-blazor.yml`), the PATCH counter is auto-incremented via a persistent state file on the self-hosted runner (`~/.brewhouse/deploy-build-state`) — it resets to 0 when MAJOR.MINOR changes. The full `X.Y.Z` version is injected into the binary via `/p:Version=X.Y.Z` and read at runtime by `VersionService` (from `AssemblyInformationalVersionAttribute`). To manually seed/reset the state file:
  ```bash
  mkdir -p ~/.brewhouse
