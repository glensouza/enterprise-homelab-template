...
| **Authentik** | VLAN 130 (`10.10.130.123`) | **`pve3`** (Node 2) | 4 | 8192 MB (+2048 MB swap) | *None* | SSO — bare-metal from-source build (Rust/Go/Node/Python), sized like community-scripts' own defaults rather than trimmed (ADR 48/59: don't undersize an LXC that does real compile/link work) |
