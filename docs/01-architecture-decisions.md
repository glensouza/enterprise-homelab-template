...
* **Rationale:** MassTransit v9 went commercial. Wolverine provides the same transport-agnostic, `IBus`-style abstraction (`IMessageBus`) while remaining free/open-source and lighter weight — handlers (`ProcessBidHandler`) are plain static classes discovered automatically and never change when the transport changes, proving the abstraction is real rather than aspirational. If the application migrates to Azure or AWS, the transport swap is one config value; the SQS/Service Bus code paths are exercised against the Floci emulator in local dev and the PR preview stack rather than only against real cloud accounts. The durable outbox is what makes "kill the broker, publish, restart the broker, watch it flush" a real demo rather than a claim: Postgres is already a hard dependency of the app, so this is one config block, not new infrastructure. Wolverine's own envelope-storage schema (`wolverine.*`, distinct from the app's EF Core schema) is auto-applied on startup only in `Development` (`UseResourceSetupOnStartup()`) for local/Aspire convenience; production applies it as a deliberate one-time step for the same reason ADR 11 forbids `Database.Migrate()` on boot — two HA nodes must never race on schema creation. That step is the **Apply Wolverine Envelope Schema** stage in `deploy-blazor.yml`, which runs `dotnet RoadrunnerAuction.dll db-apply` immediately after the EF Core migration bundle and before any node is deployed. The CLI exists because `Program.cs` ends in `RunJasperFxCommands(args)` rather than `app.Run()`; with no arguments that starts the web host identically, so the systemd unit is unaffected.
