...
├── inventory/
│   ├── hosts.yml                 # static inventory (web / postgres groups)
│   ├── terraform-hosts.yml       # generated by terraform apply (git-ignored)
│   └── group_vars/               # NAS server, mounts, pgBackRest settings
└── roles/
...
5. **Apply:**
   ...
   terraform init
   terraform plan
   terraform apply   # creates VLANs, firewall rules, and all 9 LXCs,
