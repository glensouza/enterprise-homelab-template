...
The systemd unit runs from `/var/www/brewhouse/current`, so rollback never modifies files — it only moves the symlink and restarts the service.
...
```bash
ssh root@10.10.110.101
ln -sfn /var/www/brewhouse/releases/<SHA> /var/www/brewhouse/current
systemctl restart blazor-app.service
curl -fsS http://localhost:5000/health
# repeat on 10.10.110.102 after verifying 01 is healthy
```
...
```text
/mnt/synology/postgres-data/backups/brewhouse_db-<sha8>-<timestamp>.sql.gz   (on the Postgres LXC's NAS mount)
```
...
2. Stop both app services: `ssh root@10.10.110.10{1,2} systemctl stop blazor-app.service`
3. On the Postgres LXC, restore into a fresh database and swap:
```bash
ssh root@10.10.120.110
sudo -u postgres createdb brewhouse_db_restore
gunzip -c /mnt/synology/postgres-data/backups/brewhouse_db-<sha8>-<timestamp>.sql.gz | sudo -u postgres psql brewhouse_db_restore
# VERIFY the restored data before proceeding.
# Only after verification, explicitly rename the databases:
sudo -u postgres psql -c 'ALTER DATABASE brewhouse_db RENAME TO brewhouse_db_failed;'
sudo -u postgres psql -c 'ALTER DATABASE brewhouse_db_restore RENAME TO brewhouse_db;'
```

4. Start the app services and verify `/health`.
5. Keep `brewhouse_db_failed` until you are confident, then drop it manually.
...
- **Verify:** `sudo -u postgres pgbackrest --stanza=brewhouse info` shows backups and archive ranges.
...
1. Stop both app services: `ssh root@10.10.110.10{1,2} systemctl stop blazor-app.service`
2. On the Postgres LXC, stop PostgreSQL and restore to the target time:
```bash
ssh root@10.10.120.110
systemctl stop postgresql
sudo -u postgres pgbackrest --stanza=brewhouse --delta \
...
- **DB dumps:** pruned automatically by the `pg-dump-prune` systemd timer on the Postgres LXC (`10.10.120.110`) — runs daily, deletes `brewhouse_db-*.sql.gz` older than 30 days, and refuses to run if the NAS mount is down. See `src/systemd/pg-dump-prune.{sh,service,timer}`; retention is configurable via the service `Environment=RETENTION_DAYS=` (installed by Ansible, `docs/04-lxc-provisioning.md` section 3).
